    ...
    try {
        ...
        if (customerError) {
            ...
            return NextResponse.json(
                { error: 'Gagal menyimpan data pelanggan', details: customerError.message },
                { status: 500 }
            );
        }
        ...
        const generateTicket = (type: string, customerName: string) => {
            ...
            const namePrefix = (customerName || 'XXX').toUpperCase().slice(0, 3).padEnd(3, 'X');
            const randomNum = Math.floor(1000 + Math.random() * 9000);
            return `${prefix} ${namePrefix} ${randomNum}`;
        };

        const now = new Date();
        const insertedOrders: { id: string; nomor_tiket: string; jenis_tugas: string }[] = [];
        ...
        for (const type of jenisTugasArray) {
            ...
            const { data: permintaanData, error: permintaanError } = await supabase
                ...
                .insert({
                    ...
                    waktu_penjemputan: waktuPenjemputan ? new Date(waktuPenjemputan).toISOString() : null,
            ...
            if (permintaanError) {
                ...
                return NextResponse.json(
                    { error: 'Gagal menyimpan permintaan', details: permintaanError.message },
                    { status: 500 }
                );
            }

            insertedOrders.push({ id: permintaanData.id, nomor_tiket: nomorTiket, jenis_tugas: type });

            // 3. Insert order_items for each permintaan
            const produkFinal = produkLayanan === 'lainnya' ? produkLayananManual : produkLayanan;
            ...
            if (itemError) {
                ...
                return NextResponse.json(
                    { error: 'Gagal menyimpan detail pesanan', details: itemError.message },
                    { status: 500 }
                );
            }
        }
