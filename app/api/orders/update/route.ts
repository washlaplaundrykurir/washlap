...
export async function PUT(request: NextRequest) {
    try {
        ...
        const { orderId, customerId, nama, phone, alamat, mapsLink, produk, layanan, parfum, statusId, courierId, nomorNota, waktuPenjemputan } = await request.json();

        if (!orderId || !customerId) {
            return NextResponse.json({ error: 'Order ID dan Customer ID diperlukan' }, { status: 400 });
        }
        ...
        if (statusId !== undefined) {
            ...
            if (statusId === 2) updatePermintaanData.waktu_assigned = new Date().toISOString();
            if (statusId === 6) updatePermintaanData.waktu_selesai = new Date().toISOString();
        }

        if (courierId !== undefined) updatePermintaanData.courier_id = courierId;
        if (nomorNota !== undefined && nomorNota !== '') updatePermintaanData.nomor_nota = nomorNota;
        if (waktuPenjemputan !== undefined && waktuPenjemputan !== '') {
            updatePermintaanData.waktu_penjemputan = new Date(waktuPenjemputan).toISOString();
        }
