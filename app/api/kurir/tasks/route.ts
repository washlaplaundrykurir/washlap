...
export async function GET(request: NextRequest) {
    try {
        ...
        const todayTasks = orders?.filter(o => new Date(o.waktu_order) >= todayStart).length || 0;
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!id || status_id === undefined) {
            return NextResponse.json({ error: 'Order ID dan status diperlukan' }, { status: 400 });
        }
        ...
        if (order.courier_id !== userId) {
            return NextResponse.json({ error: 'Tidak memiliki akses ke order ini' }, { status: 403 });
        }

        // Update the order
        const updateData: Record<string, unknown> = { status_id };
        if (status_id === 6) updateData.waktu_selesai = new Date().toISOString();
        if (status_id === 3 || status_id === 5) updateData.waktu_kurir_selesai = new Date().toISOString();
