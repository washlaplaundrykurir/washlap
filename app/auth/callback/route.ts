...
import { createSupabaseAdmin, supabaseUrl, supabaseServiceKey } from '@/utils/supabase/server'
    ...
    if (code) {
        ...
        if (!error && data.session) {
            ...
            try {
                ...
                console.log(`[AUTH] Whitelist check result:`, { existingUser, queryError });

                if (!existingUser) {
                    // Email not in whitelist - user not authorized
                    console.log(`[AUTH] Email ${userEmail} NOT in whitelist. Deleting user ${userId}...`);
                    ...
                    return NextResponse.redirect(`${origin}/login?error=email_not_registered`)
                }

                console.log(`[AUTH] Email ${userEmail} found in whitelist. Proceeding...`);
            } catch (err) {
