...
import { ClipboardList, RefreshCw, PartyPopper, User, Phone, MapPin, ExternalLink, ShoppingBasket, FileText, X, Check } from "lucide-react";
...
export default function KurirTasksPage() {
    ...
    const getCompleteLabel = (jenisTugas: string) => {
        return jenisTugas === "JEMPUT" ? <><Check size={16} /> Sudah Jemput</> : <><Check size={16} /> Sudah Antar</>;
    };
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <Button variant="flat" size="sm" onClick={fetchTasks} isLoading={isLoading}>
            ...
            <Tabs
                ...
                className="mb-6"
                classNames={{
                    tabList: "bg-white/60 dark:bg-white/15 backdrop-blur-xl border border-black/10 dark:border-white/30",
                }}
            >
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    <div className="text-sm">
                                        <p className="text-gray-900 dark:text-white font-medium flex items-center gap-1">
                                            <User size={14} /> {order.customers?.nama_terakhir || "Unknown"}
                                        </p>
                                        <a
                                    ...
                                    {order.order_items?.[0] && (
                                        <div className="text-xs text-gray-500 dark:text-white/50 bg-gray-100 dark:bg-white/5 p-2 rounded-lg">
                                            <ShoppingBasket size={12} className="inline mr-1" /> {order.order_items[0].produk_layanan} • {order.order_items[0].jenis_layanan} • {order.order_items[0].parfum}
                                        </div>
                                    )}
                                    ...
                                    {order.catatan_khusus && (
                                        <div className="text-xs bg-yellow-100 dark:bg-yellow-500/20 p-2 rounded-lg text-yellow-800 dark:text-yellow-200">
                                            <FileText size={12} className="inline mr-1" /> {order.catatan_khusus}
                                        </div>
                                    )}
