...
export default function KurirPage() {
    ...
    const [stats, setStats] = useState<Stats>({ todayTasks: 0, pendingTasks: 0, completedTasks: 0, totalTasks: 0 });
    ...
    const fetchTasks = async () => {
        try {
            ...
            setStats(result.stats || { todayTasks: 0, pendingTasks: 0, completedTasks: 0, totalTasks: 0 });
        } catch (err) {
    ...
    const updateStatus = async (orderId: string, newStatusId: number) => {
        try {
            ...
            showToast("success", newStatusId === 7 ? "Tugas dibatalkan" : "Tugas selesai!");
            fetchTasks();
        } catch (err) {
            showToast("error", err instanceof Error ? err.message : "Gagal update status");
        } finally {
    ...
    const getCompleteLabel = (jenisTugas: string) => {
        return jenisTugas === "JEMPUT" ? <><CheckCircle size={16} /> Sudah Jemput</> : <><CheckCircle size={16} /> Sudah Antar</>;
    };

    // Check if task can be completed or cancelled
    const isCompleted = (statusId: number) => statusId === 6;
    const isCancelled = (statusId: number) => statusId === 7;
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-600 dark:text-white/70">Siap untuk mengirim pesanan hari ini?</p>
            </div>
            {/* Stats Cards */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <Card className="backdrop-blur-xl bg-white/60 dark:bg-white/15 border border-black/10 dark:border-white/30">
                    <CardBody className="p-4 text-center">
                        ...
                        <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.todayTasks}</p>
                    </CardBody>
                </Card>
                <Card className="backdrop-blur-xl bg-white/60 dark:bg-white/15 border border-black/10 dark:border-white/30">
                    <CardBody className="p-4 text-center">
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <Button variant="flat" size="sm" as={Link} href="/kurir/history">
                    Lihat Riwayat →
                </Button>
            </div>
                            ...
                            <CardBody className="p-4">
                                <div className="flex flex-col gap-3">
                                    ...
                                    <div className="flex items-center justify-between flex-wrap gap-2">
                                        ...
                                        <Chip size="sm" color={statusColors[order.status_id] || "default"}>
                                    ...
                                    <div className="text-sm">
                                        <p className="text-gray-900 dark:text-white font-medium flex items-center gap-1">
                                            <Hand size={14} /> {order.customers?.nama_terakhir || "Unknown"}
                                        </p>
                                    ...
                                    {order.order_items?.[0] && (
                                        <div className="text-xs text-gray-500 dark:text-white/50">
                                            <Package size={12} className="inline mr-1" /> {order.order_items[0].produk_layanan} • {order.order_items[0].jenis_layanan}
                                        </div>
                                    )}
                                    ...
                                    {order.catatan_khusus && (
                                        <div className="text-xs bg-yellow-100 dark:bg-yellow-500/20 p-2 rounded-lg text-yellow-800 dark:text-yellow-200">
                                            <ClipboardList size={12} className="inline mr-1" /> {order.catatan_khusus}
                                        </div>
                                    )}
                                    ...
                                    <div className="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-white/10">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            >
                                                <CheckCircle size={16} className="rotate-45" /> Dibatalkan
                                            </Button>
                                            ...
                                            <Button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    updateStatus(order.id, order.jenis_tugas === "JEMPUT" ? 3 : 5);
                                                }}
                                            >
